...
import cn.patterncat.helper.sql.query.*;
import cn.patterncat.helper.sql.support.DatabaseType;
...
public class QueryDslJdbcTemplate implements QueryDslJdbcOperations {
    ...
    public QueryDslJdbcTemplate(JdbcTemplate jdbcTemplate) {
        ...
        if (dbType == DatabaseType.DERBY) {
            ...
            this.dialect = new PostgreSQLTemplates();
        }
    ...
    public <T> List<T> query(final SQLQuery sqlQuery, final Expression<T> expression) {
        List<T> results = jdbcTemplate.execute(new ConnectionCallback<List<T>>() {
            public List<T> doInConnection(Connection con) throws SQLException,
                ...
                SQLQuery liveQuery = sqlQuery.clone(con);
                try {
                    return liveQuery.select(expression).fetch();
                } catch (QueryException qe) {
                    throw translateQueryException(qe, "SQLQuery", liveQuery.toString());
                }
            }});
    ...
    public <T> QueryResults<T> queryResults(final SQLQuery sqlQuery, final Expression<T> expression) {
        QueryResults<T> results = jdbcTemplate.execute(new ConnectionCallback<QueryResults<T>>() {
            public QueryResults<T> doInConnection(Connection con) throws SQLException,
                    DataAccessException {
                SQLQuery liveQuery = sqlQuery.clone(con);
                try {
                    return liveQuery.select(expression).fetchResults();
                } catch (QueryException qe) {
                    throw translateQueryException(qe, "SQLQuery", liveQuery.toString());
                }
            }});
    ...
    private ResultSet queryForResultSet(SQLQuery liveQuery, Expression<?>[] projection) throws DataAccessException {
        ResultSet resultSet;
        try {
            resultSet = liveQuery.select(projection).getResults();
        } catch (QueryException qe) {
            throw  translateQueryException(qe, "SQLQuery", liveQuery.toString());
        }
        return resultSet;
